...
/**
 * Higher-level utility that combines warehouse loading with React rendering
 * This is a convenient function for simple use cases
 *
 * @param warehouseUrl - URL of the warehouse
 * @param containerId - ID of the DOM element to render into
 * @param renderFunction - Function that takes loaded components and returns JSX
 */
export async function bootstrapReactWarehouse(
  warehouseUrl: string,
  containerId: string,
  renderFunction: (
  ...
  try {
    // Load all components from the warehouse
    const components = await loadReactWarehouse(warehouseUrl);

    // Create React root and render
    const root = createRoot(container);
    const app = renderFunction(components);
    root.render(app);

    return { components, root };
  } catch (error) {
    console.error("❌ Failed to bootstrap React warehouse:", error);
    ...
    root.render(
      React.createElement(
        ...
        React.createElement("h1", null, "❌ Error Loading Application"),
        React.createElement(
          "p",
          null,
          `Failed to load components: ${errorMessage}`
        )
      )
    );
    throw error;
