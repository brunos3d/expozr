// React Host - Simple approach to test React component loading
async function loadReactComponents() {
  try {
    console.log("🚀 React Host application starting...");

    // Test basic connectivity first
    console.log("🔍 Testing React warehouse connectivity...");
    ...
    if (!inventoryResponse.ok) {
      throw new Error(
        `React warehouse not accessible: ${inventoryResponse.status}`
      );
    }

    const inventory = await inventoryResponse.json();
    console.log(
      "✅ React warehouse accessible. Available components:",
